...
flages.DEFINE_string('model_dir',default='',help=('Directory for model data.'))

flags.DEFINE_string('experiment',default='xpos',help=('Experiment name.'))

flags.DEFINE_string(batch_size,default=64,help=('Batch size for training'))

flags.DEFINE_integer('eval_frequency',default=100,help=('Frequency of eval during training, e.g. every 1000 steps.'))
...
def compute_weighted_cross_entropy(logits,targets,weights=None):
    ...
    onehot_targets=common_utils.onehot(targets,logits.shape[-1])   #[batch_size, length, num_classes]    
...
def compute_metrics(logits,labels,weights):
    ...
    acc,_=comput_weighted_accuracy(logits,labels,weights)
    metrics={
        'loss':loss,
        'accuracy':accuracy,
...
def main(argv):
    ...
    for step,batch in zip(range(num_train_steps),train_iter):
        ...
        if (step+1)%eval_freq==0:
            ...
            summmary=jax.tree_map(lambda x:x/denominator,metrics_sums)
            ...
            for eval_batch in eval_iter:
                ...
                if cur_pred_batch_size!=batch_size:
                    ...
                    eval_batch=jax.tree_map(
                        lambda x:padding(x,batch_size),eval_batch)
